...
Dir[File.join(repo_root, "specs", "epics", "**", "epic.yaml")].sort.each do |epic_path|
  document = YAML.safe_load(File.read(epic_path), permitted_classes: [], permitted_symbols: [], aliases: false) || {}
...
Dir[tasks_glob].sort.each do |tasks_path|
  document = YAML.safe_load(File.read(tasks_path), permitted_classes: [], permitted_symbols: [], aliases: false) || {}
