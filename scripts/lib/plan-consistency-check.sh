...
VERIFY_EXTRACTOR="$SCRIPT_LIB_DIR/extract-story-verify.rb"
if [[ ! -f "$VERIFY_EXTRACTOR" ]] || ! command -v ruby >/dev/null 2>&1; then
  echo "BLOCKED: Ruby standard-library YAML extractor is required" >&2
  exit 1
fi
...
for tasks in "${TASKS[@]+"${TASKS[@]}"}"; do
  ...
  if [[ -z "$spec_name" || ! -f "$spec" ]]; then
    ...
    if grep -qiE 'ambiguous|TBD|TODO|FIXME' "$spec"; then
  ...
  if ! grep -qE '^[[:space:]]*verify:' "$tasks"; then
    ...
    verify_cmd="$(ruby "$VERIFY_EXTRACTOR" "$REPO_ROOT" \
      | awk -F'\t' -v s="$sid" '$1 == s || index($1, s "-") == 1 { print $4; exit }')"
    if [[ -n "$verify_cmd" ]]; then
      ...
      report CRITICAL "Story $sid verify: could not extract a runnable command"
    fi
  fi
