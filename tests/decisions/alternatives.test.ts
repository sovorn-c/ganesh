...
test("keeps alternatives isolated with branch snapshot regression coverage", () => {
...
test("adopts one reviewed exact alternative and marks transitive impacts", () => {
  const fixture = projectFixture();
  try {
...
test("rejects concurrent stale adoption without changing destination history", () => {
  const fixture = projectFixture();
  try {
