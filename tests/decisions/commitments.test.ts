...
test("filters commitment history by branch", () => {
...
test("preserves lifecycle history inspection across revision, reopen, archive, and supersession", () => {
  const fixture = projectFixture();
  try {
...
test("separates historical approval from current readiness and persists causes", () => {
  const fixture = projectFixture();
  try {
...
test("readiness records remain inspectable after policy withdrawal regression", () => {
  const fixture = projectFixture();
  try {
