...
type DocumentApi = {
  ...
  extractDocumentSource: (handle: ProjectHandle, sourceVersionId: string, limits?: Record<string, number>) => Promise<{ status: string; derivedVersionId?: string; segments: readonly Record<string, unknown>[] }>;
...
test("e06s02 pdf page derived provenance loss", async () => {
  ...
  try {
    ...
    strictEqual(extracted.segments[0]?.pageNumber, 1);
...
test("e06s02 docx paragraph table provenance", async () => {
  ...
  try {
    ...
    const extracted = await documentApi.extractDocumentSource(fixture.handle, imported.source.artifactVersionId);
    strictEqual(extracted.status, "complete");
    strictEqual(extracted.segments.some((segment) => segment.paragraphNumber === 1), true);
    strictEqual(extracted.segments.some((segment) => segment.tableNumber === 1), true);
  } finally {
    disposeFixture(fixture);
  }
});
...
test("e06s02 corrupt document fails without derived output", async () => {
  ...
  try {
    const documentApi = await api();
    const imported = documentApi.importLocalSource(fixture.handle, createOwnerCapability("owner-test"), sourceRequest(path, "docx"));
    await throwsAsync(() => documentApi.extractDocumentSource(fixture.handle, imported.source.artifactVersionId));
  } finally {
    disposeFixture(fixture);
  }
});

async function throwsAsync(action: () => Promise<unknown>): Promise<void> {
  try {
    await action();
  } catch {
    return;
