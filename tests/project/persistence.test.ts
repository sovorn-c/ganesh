...
test("project schema survives reopen offline", () => {
...
test("artifact immutable hash and later versions", () => {
  const fixture = projectFixture();
  try {
...
test("duplicate registration cannot replace immutable bytes", () => {
  const fixture = projectFixture();
  try {
...
test("dependency inspection distinguishes missing corrupt and unavailable", () => {
  const fixture = projectFixture();
  try {
...
test("atomic registration leaves no incomplete reference", () => {
  const fixture = projectFixture();
  try {
    assert.throws(
      () => registerArtifactVersion(fixture.handle, {
