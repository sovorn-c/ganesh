...
test("schema status distinguishes supported migration and future", () => {
...
test("recovery removes temporary files and preserves complete state", () => {
  const fixture = projectFixture();
  try {
...
test("read-only offline inspection cannot roll back current state", () => {
  const fixture = projectFixture();
  try {
