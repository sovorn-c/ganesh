...
test("schema migration and validated policy types", () => {
...
test("record exact-version classification and grant with expiry or withdraw", () => {
  const fixture = projectFixture();
  try {
...
test("evaluate policy deny unclassified and out-of-scope requests", () => {
  const fixture = projectFixture();
  try {
...
test("local defaults reject forged approval and maintain history compat", () => {
  const fixture = projectFixture();
  try {
