...
test("disclosure request destination canonicalization and redacted decision", () => {
...
test("inherit effective restrictions across dependency versions and branch isolation", () => {
  const fixture = projectFixture();
  try {
...
test("route prompt attach summary compact snippet embed telemetry export diagnostic analysis through gateway", () => {
  const fixture = projectFixture();
  try {
...
test("cross-branch revoke blocks future requests with no fallback and offline redact decisions", () => {
  const fixture = projectFixture();
  try {
