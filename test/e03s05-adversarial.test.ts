...
function makeFixture(): { root: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "ganesh-e03s05-"));
  return {
    root,
    cleanup: () => {
      try {
...
test("e03s05 declass transform residual audit records exact input versions and reviewable status", () => {
...
test("e03s05 authorize current restriction lifecycle gates validate transformed disclosure against policy", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
...
test("e03s05 adversarial forged credential cross.project script side.effect fixtures fail at actual boundaries", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
...
test("e03s05 redact regression provenance inspect audit packet exposes safe decisions without payloads", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
