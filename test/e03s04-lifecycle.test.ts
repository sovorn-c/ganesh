...
function makeFixture(): { root: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "ganesh-e03s04-"));
  return {
    root,
    cleanup: () => {
      try {
...
test("e03s04 operation checkpoint fence quarantine lifecycle records survive offline reload", () => {
...
test("e03s04 dispatch external effect gates evaluate current policy capability and input protection", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
...
test("e03s04 revoke cancel fence blocks late branch candidate accept and quarantines output", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
...
test("e03s04 resume stale branch snapshot rechecks current policy acceptance checkpoint after restart", () => {
  const fixture = makeFixture();
  try {
    const handle = createProject({ rootPath: fixture.root, ownerId: "owner-alice" });
