...
import { disposeFixture, projectFixture } from "./e02-fixtures.js";

test("e03s03 non-serializable capability objects distinguish owner and worker scope", () => {
...
test("e03s03 protect canonical write credentials and project paths from forged roles", () => {
  const fixture = projectFixture();
  try {
...
test("e03s03 curated pi loader and inert document render disarm script macro injection", () => {
  const fixture = projectFixture();
  try {
    const workerCap = createWorkerCapabilities({
      projectId: fixture.handle.project.id,
...
test("e03s03 execute local command with ask approve full.access mode and bash guard", () => {
  const guard = {
